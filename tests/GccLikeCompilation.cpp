...
GccLikeCompilationChecks::GccLikeCompilationChecks(
  ...
  , mTinyOptionalIncludeDir(tinyOptionalIncludeDir)
  , mTinyOptionalIncludeDirCanon(weakly_canonical(tinyOptionalIncludeDir))
...
void GccLikeCompilationChecks::PrintInputOptions(std::ostream & out) const
{
  ...
  out << "\t==> TinyOptional include directory (input):     " << mTinyOptionalIncludeDir << std::endl;
  out << "\t==> TinyOptional include directory (canonical): " << mTinyOptionalIncludeDirCanon << std::endl;
...
auto GccLikeCompilationChecks::PerformCompilation(std::string const & code) const -> ExecutionResult
{
  ...
  std::string command = mExecutable.string() + " " + mCompilationFlags + " -I\"" + mTinyOptionalIncludeDirCanon.string()
  ...
  std::string command = "printf \"%s\" '" + code + "' | " + mExecutable.string() + " " + mCompilationFlags + " -I\""
                        + mTinyOptionalIncludeDirCanon.string() + "\" -c -x c++ -o /dev/null -";
#endif
